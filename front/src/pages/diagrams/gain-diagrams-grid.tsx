...
export const GainDiagramsGrid = (props: GainDiagramsProps) => {

    const service : TransactionService = new TransactionService();
    const [state, setState] = useState<GainDiagramsState>({
        data: undefined,
    ...
    useEffect(() => {
        service
            .getGains(state.dateFrom, state.dateTo)
            .then(response => {
                setState(prevState => ({
                    ...prevState,
    ...
    return <>
        ...
        <PieChart
            ...
            palette="Bright"
            title="Распределение доходов по категориям"
        ...
        >
            <Series
                ...
                valueField="amount"
                hoverStyle={{
                    border: { width: 7, visible: true, dashStyle: "solid" },
                    hatching: {
                        direction: 'right',
                        step: 5,
                        width: 5,
